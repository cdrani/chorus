...
const createIcon = ({ type, id = '', role = '', lw = 20, viewBox = '-4 -4 24 24', styles = '' }) => `
    <button 
        role="${role || type}"
        id="${(id || type) + '-btn'}"
        style="padding:0;border:none;background:none;display:flex;justify-content:center;align-items:center;cursor:pointer;${styles}"
    > 
        <svg
            width="${lw}"
            height="${lw}"
...
export const createRootContainer = () => `
    <div id="chorus">
        ...
        <div id="controls" style="display:flex;width:100%;height:32px;align-items:center;justify-content:space-between">
            <div id="left-controls" style="display:flex;justify-content:space-between;width:40px;margin-right:20px;">
                ${createIcon({ type: 'heart', role: 'save/unsave', viewBox: '-6 -6 24 24' })}
                ${createIcon({ type: 'block', role: 'block-track' })}
            </div>
